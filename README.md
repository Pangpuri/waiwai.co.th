...
- หน้าแรกมี **แถบภาพสไลด์** (จางข้ามภาพ + ซูมช้า ๆ วนไป) และมี **ปุ่มหยุด/เล่นต่อ** กำกับ
  เพราะเนื้อหาที่เลื่อนเองต้องหยุดได้ (WCAG 2.2.2) — ภาพชุดนี้เป็นภาพตัวอย่างรอการตลาดอนุมัติ
  - **ข้อความ hero อยู่ใต้แถบภาพ ไม่ทับบนภาพ** (การตลาดขอ รอบที่ 21 — เดิมทับกันจนอ่านยาก/บังสินค้า)
    · แถบภาพ: จอเล็ก 24rem · lg 36rem (`min-h-[…]` ใน `features/home/ui/hero.tsx`)
    · สีข้อความอยู่บนพื้นหน้าเว็บ → คำหลักแดงแบรนด์ · คำรอง `text-accent` (แดงเข้ม) · เนื้อหา `text-fg-muted`
      **ห้ามใช้เหลืองแบรนด์กับข้อความบนพื้นสว่าง** (ได้ ~1.4:1) — มีเทสต์คอนทราสต์คุมทั้งสองโหมด
  - ภาพ/alt ปรับที่ `features/home/slides.ts` + `lib/i18n/messages/areas/<locale>/home.ts` (`hero.slides`)
  - ไฟล์ภาพทุกไฟล์ใน `public/slide/` **ต้องถูกอ้างใน `HERO_SLIDES`** (มีเทสต์เตือนเรื่องไฟล์ค้าง)
  - **สเปกภาพสไลด์ที่แนะนำ** (กรอบ hero กว้างเต็มจอ + `object-cover` → ภาพถูกย่อ/ขยายตามจอจริง)
    - ขนาด: **3840×2160 (16:9) ปลอดภัยทุกจอ** · ขั้นต่ำ 2560×1440 · **ไม่ต้องเกิน 3840 กว้าง**
      (ค่า default ของ Next สร้าง variants สูงสุด 3840px — ไฟล์ใหญ่กว่านั้นไม่ได้ทำให้คมขึ้น)
    - ขนาดไฟล์: ~400KB–1.5MB ต่อรูป · JPEG q82–88 (ภาพที่มีตัวอักษรใช้ q88) · sRGB
    - สัดส่วน 16:9 ปลอดภัยสุด (กรอบ hero สูงไม่เท่ากันในแต่ละจอ) · เก็บจุดสนใจไว้กลางภาพเพราะถูกครอป
      (ตำแหน่งครอปตั้งต่อรูปได้ที่ `objectPosition` ใน `features/home/slides.ts`)
    - **ห้ามเอาไฟล์เล็กไปขยายเอง** (นุ่ม + ไฟล์อ้วน) — ขอไฟล์ต้นฉบับจากดีไซเนอร์
    - ที่มา: hero ต้องคลุมกล่องถึง 3840×1440 บนจอ Retina/4K · ภาพปัจจุบัน (1693×845 · 1280×720 · 1200×675)
      ถูกขยาย 1.1–1.6 เท่าบนจอ 1920 และ 2.3–3.2 เท่าบน Retina = ต้นเหตุที่ดูนุ่ม/แตก (หนี้ใน roadmap § 9)
    - ตรวจของจริง: DevTools → Network → กรอง `_next/image` → ดูว่าเบราว์เซอร์ขอ `w=` เท่าไร
...
- **การ์ดประกาศบน hero** (แปะมุมขวาล่างของแถบภาพ · จอเล็กลอยมุมล่างของภาพ เหนือจุดบอกตำแหน่งสไลด์) — เอาไว้ "โชว์อะไรเล็ก ๆ น้อย ๆ"
